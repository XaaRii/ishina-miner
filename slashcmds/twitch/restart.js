...
module.exports = {
  ...
  async execute(interaction) {
    ...
    function response(i, msg) {
      if (isFollowUp) {
        if (!followUpMessage)
          interaction.followUp(msg).then((m) => {
            followUpMessage = m;
          });
        else
          followUpMessage
            .edit(msg)
            .catch((er) => console.log("something fucked up, " + er));
      } else if (i)
        interaction
          .reply(msg)
          .catch((er) => console.log("something fucked up, " + er));
    ...
    tmmachines.find({ tmowner: authorid }, async function (err, docs) {
      if (docs.length < 1)
        return response(
          1,
          "Sorry, but you don't own any miner. Though, you can register one using `/twitch create <username>`",
        );
      if (!docs[0].tmpassworded)
        return response(
          1,
          "Your miner is missing cookies file. Please use `/twitch auth` to finish the setup",
        );
      if (!recentBlock.includes(authorid)) {
        recentBlock.push(authorid);
        setTimeout(() => {
          recentBlock = recentBlock.filter((x) => x !== authorid);
        }, 30000);
      }
      if (!isFollowUp) await interaction.deferReply();
      exec(
        `screen -ls | grep "tm-"| awk '{print $1}' | cut -d. -f 2 | cut -c 4-`,
        async function (error, stdout, stderr) {
          ...
          function runFileBuild(victlist, n, d) {
            ...
            let oldFile = fs.readFileSync(
              "./twitchminers/run" + authorid + ".py",
              "utf8",
            );
            if (!oldFile) {
              return response(
                0,
                "error: Your file seems to be missing *somehow*. Contact Pawele, he will help ya.",
              );
            }
            ...
            exec(
              `cd twitchminers && screen -S tm-${authorid} -d -m bash starter.sh ${authorid}`,
            );
            setTimeout(() => {
              return spectR(0, 1);
            }, 1000);
          ...
          async function spectR(i, mode) {
            exec(
              `screen -S tm-${authorid} -X hardcopy "./templogs/${authorid}.log" && sleep 1 && tac ./twitchminers/templogs/${authorid}.log | grep -m 10 '[[:blank:]]' | tac`,
              async function (e, o, oe) {
                var finito = false;
                if (e) {
                  // process stopped
                  if (!mode) {
                  ...
                  mode
                    ? embed.setDescription(
                        "Twitch miner is not running. (" + i + "/6)",
                      )
                  ...
                  if (mode) {
                    ...
                    if (o.includes("Loading data for")) {
                      ...
                      embed.setDescription(
                        "Twitch miner couldn't start! (maybe you changed your password?)",
                      );
                      finito = true;
                      exec("screen -S tm-" + authorid + " -X stuff $'\003'");
                ...
                if (mode) {
                  var cont = running
                    ? (cont = "Twitch miner is running!")
                    : "Starting up...";
                  if (!viewMsg) {
                    viewMsg = await response(0, {
                      content: cont,
                      embeds: [embed],
                    });
                  ...
                  const msgObj = {
                    content:
                      "Initiated shutdown... please wait a moment for it to close all channels.",
                    embeds: [embed],
                  };
                  response(0, msgObj);
          ...
          function docsUpdate(runValue, pwValue) {
            ...
            tmmachines.update(
              { tmowner: authorid },
              { $set: { tmrunning: runValue, tmpassworded: pwValue } },
            );
          }
        },
      );
