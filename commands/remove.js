	...
	execute(message, args) {
		...
		tmmachines.find({ tmowner: message.author.id }, function (err, docs) {
			...
			for (let i = 0; i < args.length; i++) {
				tmvictimlist.remove({ tmusername: docs[0].tmusername, tmvictim: args[i].toLowerCase() }, function (err) {
